...
import dm.sime.com.kharetati.view.adapters.FileListAdapter;
...
public class FilePickerDialog extends Dialog implements AdapterView.OnItemClickListener {
    ...
    private NotifyItemChecked notifyChecked;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (size == 0) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                color = context.getResources().getColor(R.color.maroon_light, context.getTheme());
            } else {
        ...
        mFileListAdapter = new FileListAdapter(internalList, context, properties);
         notifyChecked = new NotifyItemChecked() {
            @Override
        ...
        mFileListAdapter.setNotifyItemCheckedListener(notifyChecked);
    ...
    public void show() {
        if (!Utility.checkStorageAccessPermissions(context)) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                ((Activity) context).requestPermissions(AppConstants.PERMISSIONS_STORAGE, AppConstants.REQUEST_READ_EXTERNAL_STORAGE);
            }
        } else {
            super.show();
            positiveBtnNameStr = positiveBtnNameStr == null ?
